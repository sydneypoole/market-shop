...
async function publish() {
  ...
  try {
    ...
    await load()
  } catch (e) { error.value = e instanceof SyntaxError ? '原始规则参数格式无效' : (e as Error).message }
...
function summary(rule: Rule) {
  try {
    const value = JSON.parse(rule.parametersJson) as Record<string, unknown>
    return Object.entries(value)
      .slice(0, 3)
      .map(([key, item]) => `${ruleParameterLabel(key)}：${ruleParameterValue(key, item)}`)
      .join(' · ')
  } catch { return '规则参数暂时无法解析' }
}

function parameterDetails(parametersJson: string) {
  try {
    const value = JSON.parse(parametersJson) as Record<string, unknown>
    return Object.entries(value)
      .map(([key, item]) => `${ruleParameterLabel(key)}：${ruleParameterValue(key, item)}`)
      .join('\n')
  } catch {
...
<template>
  <div>
    <div class="page-title"><div><h1>动态规则版本</h1><p>通过业务字段配置升级、人数、积分、复购释放和降级规则，高级模式仍可查看原始参数。</p></div><button class="primary" @click="publishing = true">发布新版本</button></div>
    ...
    <div class="card table-wrap"><table><thead><tr><th>规则编码</th><th>版本</th><th>类型</th><th>状态</th><th>生效时间</th><th>业务摘要</th><th>操作</th></tr></thead><tbody>
      <tr v-for="rule in rules" :key="rule.id"><td><b>{{ rule.ruleCode }}</b></td><td>第 {{ rule.version }} 版</td><td>{{ ruleTypeLabel(rule.ruleType) }}</td><td><span class="tag" :class="{green:rule.status === 'ACTIVE'}">{{ ruleStatusLabel(rule.status) }}</span></td><td>{{ dateTime(rule.effectiveFrom) }}</td><td class="summary">{{ summary(rule) }}</td><td class="actions"><button class="secondary" @click="openCompare(rule)">详情 / 对比</button><button v-if="new Date(rule.effectiveFrom) > new Date() && rule.status === 'ACTIVE'" class="danger" @click="cancel(rule)">取消</button></td></tr>
    </tbody></table></div>
    ...
    <div v-if="publishing" class="modal-mask" @click.self="publishing = false">
      <form class="modal rule-modal card" @submit.prevent="publish">
        ...
        <div class="grid">
          <div class="field"><label>规则类型</label><select v-model="form.ruleType"><option v-for="option in ruleTypeOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select></div>
        ...
        <section v-if="form.ruleType === 'SELF_ORDER_TASK'" class="fields"><div class="field"><label>完成订单金额门槛（分）</label><input v-model.number="fields.minimumCompletedOrderAmountFen" type="number" min="1" /></div><div class="field"><label>升级目标等级</label><select v-model="fields.targetLevel"><option value="EXPERIENCE_OFFICER">{{ memberLevelLabel('EXPERIENCE_OFFICER') }}</option><option value="SUPER_MEMBER">{{ memberLevelLabel('SUPER_MEMBER') }}</option></select></div></section>
        <section v-if="form.ruleType === 'DIRECT_REFERRAL_TASK'" class="fields"><div class="field"><label>有效直属人数</label><input v-model.number="fields.requiredCompletedDirectReferrals" type="number" min="1" /></div><div class="field"><label>新会员订单门槛（分）</label><input v-model.number="fields.minimumReferralOrderAmountFen" type="number" min="1" /></div><div class="field"><label>新会员所需等级</label><select v-model="fields.requiredReferralLevel"><option value="SUPER_MEMBER">{{ memberLevelLabel('SUPER_MEMBER') }}</option><option value="EXPERIENCE_OFFICER">{{ memberLevelLabel('EXPERIENCE_OFFICER') }}</option></select></div></section>
        ...
        <section v-if="form.ruleType === 'INACTIVITY_DOWNGRADE'" class="fields"><div class="field"><label>连续无业绩（月）</label><input v-model.number="fields.inactiveMonths" type="number" min="1" max="60" /></div><div class="field"><label>原等级</label><select v-model="fields.sourceLevel"><option value="DIVIDEND_MEMBER">{{ memberLevelLabel('DIVIDEND_MEMBER') }}</option><option value="SUPER_MEMBER">{{ memberLevelLabel('SUPER_MEMBER') }}</option></select></div><div class="field"><label>降级目标</label><select v-model="fields.targetLevel"><option value="SUPER_MEMBER">{{ memberLevelLabel('SUPER_MEMBER') }}</option><option value="EXPERIENCE_OFFICER">{{ memberLevelLabel('EXPERIENCE_OFFICER') }}</option><option value="BASIC">{{ memberLevelLabel('BASIC') }}</option></select></div></section>
        ...
        <button type="button" class="advanced" @click="advanced = !advanced">{{ advanced ? '使用业务表单重新生成原始参数' : '高级：查看或编辑原始参数' }}</button>
        <div v-if="advanced" class="field"><label>原始参数（结构化数据）</label><textarea v-model="form.parametersJson" rows="12" required /></div>
        <pre v-else>{{ parameterDetails(form.parametersJson) }}</pre>
    ...
    <div v-if="compare" class="modal-mask" @click.self="compare = undefined">
      <section class="modal compare-modal card">
        <div class="modal-title"><div><h2>{{ compare.ruleCode }} · 第 {{ compare.version }} 版</h2><p>选择同编码的其他版本进行参数对照。</p></div><button class="secondary" @click="compare = undefined">关闭</button></div>
        <select v-model="compareTarget"><option :value="undefined">不对比</option><option v-for="rule in rules.filter(item => item.ruleCode === compare?.ruleCode && item.id !== compare?.id)" :key="rule.id" :value="rule">第 {{ rule.version }} 版 · {{ ruleStatusLabel(rule.status) }}</option></select>
        <div class="compare-grid"><div><b>当前版本</b><pre>{{ parameterDetails(compare.parametersJson) }}</pre></div><div v-if="compareTarget"><b>对比版本 · 第 {{ compareTarget.version }} 版</b><pre>{{ parameterDetails(compareTarget.parametersJson) }}</pre></div></div>
      </section>
    </div>
  </div>
</template>
