...
import {
  ...
  type PublishableRuleType,
  type RuleVersionLike
} from '../rules/rule-parameters'
import { notifyError, notifySuccess } from '../toast'

type Rule = RuleVersionLike
type RuleType = PublishableRuleType
...
const baseline = computed(() => resolveRuleBaseline(rules.value, form.ruleCode, form.ruleType))
const currentRule = computed(() => {
  const value = baseline.value
  return value.state === 'valid' ? value.rule : value.state === 'invalid' ? value.rule : undefined
})
const editorLocked = computed(() => baseline.value.state === 'invalid')
const baselineError = computed(() => baseline.value.state === 'invalid' ? baseline.value.error : '')
const canOpenPublisher = computed(() => loadState.value === 'loaded' && !editorLocked.value)
...
const diffRows = computed(() => {
  ...
  const before = baseline.value.state === 'valid' ? baseline.value.parameters : {}
  const parsedAfter = parseRuleParameters(validatedPayload.value.parametersJson, form.ruleType)
  if (!parsedAfter.ok) return []
  const after = parsedAfter.value
...
function displayParameters(value?: string): Record<string, unknown> {
  if (!value) return {}
  const parsed = parseRuleParameterObject(value)
...
function hydrateFromCurrent() {
  ...
  const source = state.state === 'valid' ? state.parameters : parameters.value
  ...
  form.parametersJson = JSON.stringify(source, null, 2)
}
...
async function load() {
  ...
  loadError.value = ''
  try {
    const loaded: unknown = await adminApi<unknown>('/rules')
    if (!isRuleVersionList(loaded)) throw new Error('服务端规则列表结构无效')
    rules.value = loaded
...
function openPublisher() {
  if (!canOpenPublisher.value) return
  publishReason.value = ''
...
async function validateDraft() {
  if (editorLocked.value || validateBusy.value || !publishReason.value.trim()) return
...
async function publishValidated() {
  ...
  if (editorLocked.value || !payload || validatedSignature.value !== draftSignature.value || publishSubmitting.value) return
  ...
  try {
    ...
    committed = true
    await load()
    const readback = verifyPublishedRuleReadback(rules.value, published, form.ruleType)
    if (loadState.value !== 'loaded' || !readback.ok) {
      throw new Error(`版本已提交，但服务端最新版本回读失败：${readback.ok ? '列表加载失败' : readback.error}`)
    }
...
function summary(rule: Rule) {
  const value = displayParameters(rule.parametersJson)
...
function parameterDetails(parametersJson: string) {
  const value = displayParameters(parametersJson)
...
watch(parameters, value => {
  if (!advanced.value && baseline.value.state !== 'invalid') form.parametersJson = JSON.stringify(value, null, 2)
}, { immediate: true })
watch(baseline, value => {
...
<template>
  <div>
    <PageHeader title="动态规则版本" description="编辑草稿、服务端校验、核对字段差异并二次确认后，才发布不可变新版本。"><template #actions><button class="primary" type="button" :disabled="!canOpenPublisher" @click="openPublisher">发布新版本</button></template></PageHeader>
    ...
    <InlineAlert v-else-if="loadState === 'loaded' && editorLocked" title="当前规则版本校验失败" :message="`${baselineError}；编辑与发布已锁定，请修复服务端版本后重试读取。`" />
    ...
    <BaseDialog :model-value="publishOpen" title="编辑规则新版本草稿" description="先从当前服务端版本复制参数；修改后必须重新校验并核对差异。订单与凭证时限统一在“系统配置”维护。" width="min(940px, calc(100vw - 32px))" :submitting="validateBusy" @update:model-value="value => { if (!value) closePublisher() }"><form id="rule-draft-form" class="rule-form" @submit.prevent="validateDraft"><InlineAlert v-if="editorLocked" title="当前版本不可用" :message="`${baselineError}；不会使用本地默认值。`" /><fieldset class="editor-fields" :disabled="editorLocked"><div class="grid"><label class="field"><span>规则类型</span><select :value="form.ruleType" @change="selectRuleType(($event.target as HTMLSelectElement).value as RuleType)"><option v-for="option in publishableRuleTypeOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select></label><label class="field"><span>规则编码</span><input v-model="form.ruleCode" required /></label><label class="field"><span>生效时间（留空立即生效）</span><input v-model="form.effectiveFrom" type="datetime-local" /></label></div><InlineAlert v-if="baseline.state === 'missing'" tone="warning" title="该规则明确没有任何版本" message="将以安全表单默认值创建首个版本；请重点核对差异。" />
      ...
      <button type="button" class="advanced" @click="advanced = !advanced">{{ advanced ? '使用业务表单重新生成原始参数' : '高级：查看或编辑原始参数' }}</button><label v-if="advanced" class="field"><span>原始参数（结构化数据）</span><textarea v-model="form.parametersJson" rows="12" required /></label><pre v-else>{{ parameterDetails(form.parametersJson) }}</pre><label class="field"><span>版本发布原因</span><textarea v-model="publishReason" rows="3" maxlength="500" required placeholder="说明为什么需要调整此规则" /></label></fieldset><InlineAlert v-if="validationError" title="规则草稿校验失败" :message="validationError" /><section v-if="validatedPayload" class="diff-panel"><h3>校验通过 · 字段差异</h3><div v-if="diffRows.length"><p v-for="row in diffRows" :key="row.key"><b>{{ row.label }}</b><span>{{ row.before }}<AdminIcon name="arrow-right" :size="15" />{{ row.after }}</span></p></div><p v-else>参数与当前版本一致；仍会创建新的不可变版本。</p></section></form><template #footer="{ close }"><button class="secondary" type="button" :disabled="validateBusy" @click="close">取消</button><button v-if="!validatedPayload" class="primary" form="rule-draft-form" :disabled="editorLocked || validateBusy || !publishReason.trim()">{{ validateBusy ? '校验中…' : '校验草稿' }}</button><button v-else class="primary" type="button" :disabled="editorLocked || validatedSignature !== draftSignature" @click="publishConfirmOpen = true">进入发布确认</button></template></BaseDialog>

    <BusinessActionDialog :model-value="publishConfirmOpen" title="确认发布规则新版本" :target="`${form.ruleCode} · ${ruleTypeLabel(form.ruleType)}`" :impact="`将创建不可变新版本，生效时间为${validatedPayload ? dateTime(validatedPayload.effectiveFrom) : '未确定'}；历史订单继续使用原规则快照。`" :current-state="currentRule ? `第 ${currentRule.version} 版` : '暂无生效版本'" next-state="新版本" v-model:reason="publishReason" reason-label="版本发布原因" confirm-label="确认发布新版本" :submitting="publishSubmitting" :error="publishError" @update:model-value="value => publishConfirmOpen = value" @submit="publishValidated"><div class="confirm-diff"><p v-for="row in diffRows" :key="row.key"><b>{{ row.label }}</b><span>{{ row.before }}<AdminIcon name="arrow-right" :size="15" />{{ row.after }}</span></p><p v-if="!diffRows.length">参数无字段变化。</p></div></BusinessActionDialog>
...
<style scoped>
.boundary-note{padding:11px 14px;margin-bottom:14px;border-left:3px solid var(--color-warning);color:var(--color-warning);background:var(--color-warning-bg);font-size:13px}.summary{max-width:380px;overflow:hidden;text-overflow:ellipsis}.actions{display:flex;gap:8px}.rule-form,.editor-fields{display:grid;gap:14px}.editor-fields{min-width:0;margin:0;padding:0;border:0}.grid,.fields{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.fields{padding:13px 0;border-block:1px solid var(--color-border)}.readonly b{padding:10px;border-left:2px solid var(--color-border-strong);background:var(--color-surface-subtle)}.rule-form pre,.compare-grid pre{padding:13px;overflow:auto;color:var(--color-text);border:1px solid var(--color-border);border-radius:var(--radius-md);background:var(--color-surface-subtle);font:12px/1.6 ui-monospace,monospace}.advanced{justify-self:start;padding:0;border:0;color:var(--color-brand);background:transparent;font-weight:700}.diff-panel{padding:12px 14px;border-left:3px solid var(--color-success);background:var(--color-success-bg)}.diff-panel h3{margin:0 0 8px;font-size:15px}.diff-panel p,.confirm-diff p{display:grid;grid-template-columns:minmax(150px,.7fr) minmax(0,1fr);gap:10px;margin:0;padding:8px 0;border-bottom:1px solid var(--color-border)}.diff-panel p:last-child,.confirm-diff p:last-child{border:0}.diff-panel span,.confirm-diff span{display:flex;align-items:center;gap:6px}.confirm-diff{margin-bottom:14px}.confirm-diff span{color:var(--color-text-muted)}.compare-select{margin-bottom:16px;padding:9px;border:1px solid var(--color-border);border-radius:var(--radius-sm)}.compare-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}
