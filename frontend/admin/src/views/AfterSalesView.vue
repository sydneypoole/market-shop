  ...
  <div>
    ...
    <DetailDrawer :model-value="drawerOpen" :title="detail ? `售后 ${detail.afterSaleNo}` : '售后详情'" description="申请资料、退货信息、处理进度与服务端允许动作" width="min(860px, 100vw)" @update:model-value="value => { if (!value) closeDetail() }">
      <template v-if="detail">
        <div class="summary" aria-label="售后核心信息"><div><small>状态</small><StatusTag :tone="statusTone(detail.status)" :label="afterSaleStatusLabel(detail.status)" /></div><div><small>订单 / 买家 / 上级</small><b>#{{ detail.orderId }} / #{{ detail.applicantUserId }} / #{{ detail.superiorUserId }}</b></div><div><small>类型</small><b>{{ afterSaleTypeLabel(detail.type) }}</b></div></div>
        <div class="columns"><section><h3>申请信息</h3><dl><dt>申请原因</dt><dd>{{ detail.reason }}</dd><dt>后台意见</dt><dd>{{ detail.adminReason || '未记录' }}</dd><dt>申请时间</dt><dd>{{ dateTime(detail.createdAt) }}</dd><dt>完成时间</dt><dd>{{ dateTime(detail.completedAt) }}</dd></dl><h3>退货信息</h3><dl v-if="returnAddress(detail)"><dt>收件人</dt><dd>{{ returnAddress(detail)?.receiver }} · {{ returnAddress(detail)?.phone }}</dd><dt>地址</dt><dd>{{ returnAddress(detail)?.address }}</dd></dl><p v-else class="muted">当前流程没有退货地址。</p><dl><dt>回寄物流</dt><dd>{{ detail.returnCarrier ? `${detail.returnCarrier} ${detail.returnTrackingNo}` : '未填写' }}</dd></dl></section><section><h3>处理进度</h3><ol class="timeline"><li class="done"><b>提交售后申请</b><span>{{ dateTime(detail.createdAt) }}</span></li><li :class="{ done: detail.status !== 'PENDING_ADMIN_REVIEW' }"><b>后台审核</b><span>{{ detail.adminReason || '等待处理' }}</span></li><li :class="{ done: ['RETURN_SHIPPED','PENDING_OFFLINE_REFUND','PENDING_BUYER_REFUND_CONFIRMATION','COMPLETED'].includes(detail.status) }"><b>退货验收（如需要）</b><span>{{ detail.returnTrackingNo || '未填写' }}</span></li><li :class="{ done: ['PENDING_BUYER_REFUND_CONFIRMATION','COMPLETED'].includes(detail.status) }"><b>直属上级线下退款</b><span>系统只记录确认事实</span></li><li :class="{ done: detail.status === 'COMPLETED' }"><b>买家确认到账</b><span>{{ dateTime(detail.completedAt) }}</span></li></ol></section></div>
...
<style scoped>
.reason{max-width:240px;overflow:hidden;text-overflow:ellipsis}.summary{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));border-block:1px solid var(--color-border)}.summary>div{min-width:0;padding:13px 15px;border-right:1px solid var(--color-border)}.summary>div:last-child{border-right:0}.summary small,.summary b{display:block}.summary small{margin-bottom:5px;color:var(--color-text-muted);font-size:12px}.columns{display:grid;grid-template-columns:1fr 1fr;gap:24px}.columns section{min-width:0}.columns h3,.admin-dialog__body h3{margin:20px 0 8px;font-size:15px}dl{display:grid;grid-template-columns:90px 1fr;gap:8px;margin:0;padding:11px 13px;border-left:2px solid var(--color-border-strong);background:var(--color-surface-subtle)}dt{color:var(--color-text-muted)}dd{min-width:0;margin:0;overflow-wrap:anywhere}.muted{color:var(--color-text-muted)}.timeline{list-style:none;margin:0;padding:0}.timeline li{position:relative;padding:9px 10px 9px 25px;border-left:2px solid var(--color-border)}.timeline li:before{content:'';position:absolute;left:-6px;top:14px;width:9px;height:9px;border:2px solid var(--color-surface);border-radius:50%;background:var(--color-border-strong)}.timeline li.done{border-color:var(--color-brand)}.timeline li.done:before{background:var(--color-brand)}.timeline b,.timeline span{display:block}.timeline span{margin-top:2px;color:var(--color-text-muted);font-size:12px}.proof-state{display:flex;align-items:center;gap:10px;padding:22px 0;color:var(--color-text-muted)}.proofs article{display:flex;align-items:center;gap:12px;padding:9px 0;border-bottom:1px solid var(--color-border)}.proofs article:last-of-type{border-bottom:0}.proofs article>span{flex:1}.proofs small{display:block;color:var(--color-text-muted);margin-top:3px}.return-card{padding:12px 14px;margin-bottom:13px;border-left:3px solid var(--color-brand);background:var(--color-surface-subtle)}.return-card small,.return-card b{display:block}.return-card small{color:var(--color-text-muted)}.return-card b{margin-top:5px}.return-card p{margin:5px 0 0;line-height:1.6}
@media(max-width:720px){.summary,.columns{grid-template-columns:1fr}.summary>div{border-right:0;border-bottom:1px solid var(--color-border)}.summary>div:last-child{border-bottom:0}.proofs article{align-items:flex-start;flex-wrap:wrap}}@media(max-width:420px){dl{grid-template-columns:1fr}.proofs article .secondary{width:100%}}
</style>
