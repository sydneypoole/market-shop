...
<template>
  <div>
    <div class="page-title"><div><h1>审计日志</h1><p>按主体、动作、资源、请求号和时间范围检索不可变审计记录。</p></div><div class="title-actions"><span class="tag green">{{ total }} 条</span><button class="secondary" @click="exportCsv">导出 CSV</button></div></div>
    <div class="toolbar filters">
      <select v-model="filters.actorType"><option value="">全部主体</option><option>ADMIN</option><option>USER</option><option>SYSTEM</option></select>
      <input v-model="filters.actorId" placeholder="主体ID" /><input v-model="filters.action" placeholder="动作（精确）" />
      <input v-model="filters.resourceType" placeholder="资源类型" /><input v-model="filters.resourceId" placeholder="资源ID" />
    ...
    <div class="card table-wrap"><table><thead><tr><th>时间</th><th>主体</th><th>动作</th><th>资源</th><th>原因</th><th>请求来源</th><th>前后值</th></tr></thead><tbody>
      <tr v-for="row in rows" :key="row.id"><td>{{ dateTime(row.occurredAt) }}</td><td>{{ row.actorType }} #{{ row.actorId }}</td><td><b>{{ row.action }}</b></td><td>{{ row.resourceType }} #{{ row.resourceId }}</td><td>{{ row.reason || '—' }}</td><td><code>{{ row.requestId }}</code><small>{{ row.maskedIp || '—' }} · {{ row.userAgentSummary || '—' }}</small></td><td><details><summary>查看</summary><div class="diff"><pre>{{ pretty(row.beforeJson) }}</pre><span>→</span><pre>{{ pretty(row.afterJson) }}</pre></div></details></td></tr>
