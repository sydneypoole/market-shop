...
<template>
  <div class="pagination" aria-label="分页">
    ...
    <div>
      <button type="button" class="secondary" :disabled="page <= 1" @click="emit('change', page - 1)">上一页</button>
      <button type="button" class="secondary" :disabled="page >= pages" @click="emit('change', page + 1)">下一页</button>
    </div>
  </div>
</template>
