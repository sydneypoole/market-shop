...
test('production compose is the default and does not publish MySQL or Redis', () => {
  ...
  assert.equal(config.services.app.environment.MARKET_SHOP_STOREFRONT_BASE_URL, '')
...
test('release overlay defines an immutable, isolated candidate', () => {
  ...
  assert.equal(candidate.environment.MARKET_SHOP_STOREFRONT_BASE_URL, '')
...
test('health groups and proxy expose readiness without diagnostics', async () => {
  ...
  // Typed production validation binds callback-base-url, while
  // AuthApplicationService intentionally continues consuming the OAuth key.
  assert.match(production, /callback-base-url: \$\{MARKET_SHOP_WECHAT_CALLBACK_BASE_URL:\}/)
  assert.match(production, /oauth-callback-base-url: \$\{MARKET_SHOP_WECHAT_CALLBACK_BASE_URL:\}/)
  assert.match(production, /^  wechat:\n    oauth-callback-base-url: \$\{MARKET_SHOP_WECHAT_CALLBACK_BASE_URL:\}\n    storefront-base-url: \$\{MARKET_SHOP_STOREFRONT_BASE_URL:\}/m)
