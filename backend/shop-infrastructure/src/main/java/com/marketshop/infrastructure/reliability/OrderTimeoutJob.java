...
public class OrderTimeoutJob {
    ...
    public OrderTimeoutJob(CommerceMapper mapper, OrderTimeoutProcessor processor) {
        this.mapper = mapper;
        this.processor = processor;
    }

    @Scheduled(fixedDelayString = "${market-shop.jobs.order-timeout-delay-ms:300000}")
    public void timeout() {
        ...
        for (int index = 0; index < BATCH_LIMIT; index++) {
            try {
                ...
                LOG.warn("Order timeout failed and will be retried: {}", exception.getMessage());
                return;
            }
        }
    }
