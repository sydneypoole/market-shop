...
- Miniprogram login is strictly `POST /api/v1/auth/wechat/miniprogram/login` with body `{code}`. Public registration uses the separate `/auth/wechat/miniprogram/register` body `{code, inviteCode}`; explicit sponsor mode substitutes `sponsorClaimSecret` for `inviteCode`. The response includes a non-empty `token`; the miniprogram stores only that token and sends header `market-shop-user-token` on protected APIs.
- Public registration renders only an invitation input and one primary button. Its event calls `wx.login` once for a fresh code, submits credential-only JSON once, and goes directly home. Failure preserves the invitation but never stores or replays the code; phone, nickname and avatar are not registration inputs. The backend generates the unique platform nickname and null avatar.
- Existing-member login remains `{code}` only and goes directly home after storing the token. Returning-member profile editing is optional from the profile tab: `PUT /api/v1/membership/nickname` sends exactly `{nickname}`, `/membership/me` is authoritative, nickname saves before multipart avatar upload, and avatar retry does not repeat nickname. The page never calls login, phone, or deprecated WeChat profile APIs; temporary avatar paths remain page-memory/upload-file inputs rather than JSON, navigation, or storage values.
