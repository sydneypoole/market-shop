...
- identity-entry separation: `pages/login/login` sends only the temporary WeChat code, while `pages/register/register` sends exactly one invite or sponsor-claim credential; both pages return to the public home tab with `switchTab`, and 401 continues to `reLaunch` the login page.
