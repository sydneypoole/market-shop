...
- identity-entry separation: `pages/login/login` sends only a fresh temporary WeChat code and goes directly home. Public `pages/register/register` renders one invite input and one register button; after a local non-empty check it obtains a fresh `wx.login` code and posts exactly `{code, inviteCode}` to the separate registration route, where the backend decides invitation validity. Explicit `mode=sponsor` replaces the invite with a typed one-time claim secret without exposing a public toggle or putting the secret in a URL, QR card, share path, or `scene`.
- invitation QR card: `pages/member/invite` idempotently ensures the member's fixed invitation on load and keeps only copy, save, native share, and `user-select="{{true}}"`; it has no create, revoke, regenerate, or expiry-date UI. After an ACTIVE invitation is shown it fetches authenticated JSON `{contentType, imageBase64}` from `/membership/invitation/wxacode` and renders `data:{contentType};base64,{imageBase64}` — never a token-protected PNG URL as `<image src>`. The card shows brand `宏杉生物`, the official WeChat 小程序码, 保存二维码卡片 (`onSaveCard` writes the PNG then `saveImageToPhotosAlbum`), and native `open-type="share"` with `enableShareAppMessage`. `onShareAppMessage` title includes `宏杉生物`, path is `pages/register/register?inviteCode=...` without a leading slash, and `imageUrl` is the packaged brand logo `/assets/brand/logo.png` — never omit it (WeChat would screenshot the current page) and never use the wxacode data URI. QR fetch failure must not hide copy. Missing invitation must not call wxacode and share returns nothing. Do not add a page, FirstUI VIP qr/poster, npm, H5, or a decorative client QR as the camera-scan path.
- registration autofill: `pages/register/register` fills `inviteCode` from `query.inviteCode` first, else `query.scene` (URL-decoded, trim, slice 0..64). Explicit `mode=sponsor` ignores both. Token present still `switchTab`s home.
- registration profile collection: the profile section appears before credentials, with avatar and nickname on separate rows and the privacy confirmation directly below the nickname. It uses only `button open-type="chooseAvatar"`, `input type="nickname"`, `wx.requirePrivacyAuthorize`, and `wx.openPrivacyContract`; it never calls phone, `getUserProfile`, or `getUserInfo` capabilities. Loading disables repeat submission; failure retains the invite, while a pre-account retry always obtains another `wx.login` code and never replays an old code. After registration stores the member token, save the nickname first and then upload the in-memory avatar path; only successful completion `switchTab`s home. A nickname failure retries profile persistence without registering again, and an avatar failure retries only the avatar upload without repeating registration or the successful nickname write.
