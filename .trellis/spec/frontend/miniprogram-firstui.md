...
- identity-entry separation: `pages/login/login` sends only the temporary WeChat code, while `pages/register/register` sends exactly one invite or sponsor-claim credential. A fresh existing-member login stores the token and `reLaunch`es `pages/profile/edit`; the login page's existing-token bypass still returns directly home. Explicit home exits and completed registration/profile flows use the public home tab with `switchTab`, and 401 continues to `reLaunch` the login page.
- registration profile staging: after receiving the member token, registration uses native `chooseAvatar`, `input type="nickname"`, `getPhoneNumber`, `wx.requirePrivacyAuthorize`, and `wx.openPrivacyContract`; the protected JSON request contains only nickname plus one fresh phone code, avatar upload is a separate multipart phase, and retries never replay an invite, sponsor-claim secret, consumed phone code, or already-completed profile phase. No raw phone number or temporary avatar path may enter URL, storage, or logs.
- returning-member profile confirmation: the independent `pages/profile/edit` preloads authoritative `/membership/me`, uses only `chooseAvatar`, `input type="nickname"`, `wx.requirePrivacyAuthorize`, and `wx.openPrivacyContract`, and never calls `getPhoneNumber`, `wx.login`, `getUserProfile`, or `getUserInfo`. Skip, no-change confirmation and successful save all `switchTab` home. A changed nickname is saved before avatar upload; after nickname success an avatar retry uploads only the in-memory temporary file and never puts that path in JSON, navigation URLs, storage, or logs. The profile tab links to this same page without reauthentication.
...
- cover all 26 registered pages, including the independent profile-confirmation page, without weakening the wrapper/event/asset checks;
