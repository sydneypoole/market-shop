...
- GitHub Actions always runs miniprogram/admin tests, container contract tests, and type checking before the image job. The image-packaging path does not run Surefire: `Dockerfile` packages the JAR with `-DskipTests`, and the `quality` job must not install Java or execute `mvn … test`. Local Mockito/Surefire remains `mvn -B -ntp -f backend/pom.xml test` outside this workflow. Heavy empty-database, business-lifecycle, readiness-drill, recovery, and RustFS runtime E2E jobs are skipped for normal push/pull-request packaging and remain available through the boolean `workflow_dispatch` input `run_runtime_e2e`. When explicitly enabled, both runtime jobs must succeed before the image job; when not enabled, their `skipped` results must not suppress a quality-approved image build. Pull requests build without publishing. Pushes publish `linux/amd64` and `linux/arm64` to GHCR with branch, SHA, semantic-version, and default-branch `latest` tags.
...
- Manually enabled runtime smoke uses an isolated Compose project and ports. On an empty MySQL database it asserts Flyway startup, readiness `UP`, admin deep-link title, root 404, miniprogram login probe, a public rules API response through Nginx, and cleanup of test-only volumes.
...
- `bash -n`, ShellCheck, executable-mode checks, and operational contract tests always pass before image publication. Runtime smoke and both local/RustFS controller E2E gates run only when `workflow_dispatch.run_runtime_e2e` is enabled; once enabled, they must pass before image publication.
