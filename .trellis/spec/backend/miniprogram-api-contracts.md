...
- Invitation issue and consume eligibility is evaluated against the inviter account status, current membership-level status, and `invitation_enabled`. All four built-in levels have invitation enabled. Issue locks the inviter eligibility root before locking ordinary invitation rows and only fills a missing code; consume first resolves the code to its inviter without a lock, then locks the inviter eligibility root, locks the invitation row, asserts the inviter identity matches, and revalidates status, expiry, and use count immediately before any local registration write. A fixed ordinary invitation is `ACTIVE`, `is_bootstrap=0`, `expires_at=NULL`, and `max_uses=NULL`. Account or level invalidity makes it temporarily unusable without revoking or replacing its value; failed registration leaves all registration and invitation writes unchanged.
- The configured bootstrap invitation is persisted with its own marker, `max_uses=1`, and a terminal `REVOKED` transition on the first successful new registration. Its conditional consume update is executed only for marked rows; fixed ordinary invitations remain unlimited. V19.1 adds an exact claim-to-invitation foreign-key link and a singleton global repair guard. Existing claims and nonempty legacy databases without a linked Bootstrap invitation remain unresolved without guessing an invitation. A dedicated startup repair may clear the guard and claim flag only after locking the guard, unresolved claim, sponsor root, and exact configured code, verifying no superior relation and the deterministic earliest invitation for that root; a missing legacy claim additionally requires the independent sponsor claim secret. Until repair completes, new invitation registration fails closed with `BOOTSTRAP_INVITATION_REPAIR_REQUIRED`, including when bootstrap creation is disabled. The code exchange still occurs before the local identity transaction checks existing identity, so existing-identity idempotency and the separate sponsor-claim path remain available.
- Administrative member disable/lock and membership-level changes never mutate an ordinary invitation. Registration revalidates the locked current account and level so the same code becomes invalid while its owner is ineligible and usable again after eligibility is restored. Existing `customer_relation` rows and every persisted `trade_order.superior_user_id` remain immutable; no repair path reparents members or rewrites historical order ownership.
...
- Checkout resolves the buyer's persisted superior relation and requires that superior account and current membership level are both `ACTIVE`. It rejects an unavailable superior with `SUPERIOR_UNAVAILABLE` before order insertion or inventory reservation, rechecks the same relation under `FOR UPDATE` in the submission transaction, and never changes the stored superior ID. A non-inviting but active membership level remains available as an order superior.
...
- `ORDER_TIMERS` is snapshotted once at order submission in `trade_order_rule_snapshot`; the snapshot owns pending-order timeout days, auto-receive days, the completed-order aftersale window, and `awaitingReturnTimeoutDays` / `returnShippedTimeoutDays` / `offlineRefundTimeoutDays` / `buyerRefundConfirmTimeoutDays` (all timing fields 1–365). V19 backfills legacy orders from the rule effective at `trade_order.created_at` with deterministic version ordering and persists `trade_order.status_due_at` / `trade_after_sale.state_due_at`; `trade_order.auto_receive_at` remains the shipment due time and V19 recomputes legacy shipped rows from the historical snapshot rather than grandfathering the old value. `OrderTimeoutJob`, shipping, auto-receive, aftersale application, and the aftersale timeout worker resolve the complete immutable snapshot; SQL locks only rows whose persisted due timestamp is `<= CURRENT_TIMESTAMP(3)`, and missing or malformed snapshots fail closed with `ORDER_TIMER_SETTINGS_INVALID`. The processor cancels `PENDING_SUPERIOR`, `adminReject`s `PENDING_ADMIN_REVIEW`, and `timeoutClose`s `PENDING_SHIPMENT`, then always goes through `persistTransition` so reserved inventory is released. There is no `OrderStatus.REFUNDED`; money already collected on the last two states is an offline ops refund.
...
- `currentInvitation` is read-only and returns no invitation when one does not exist. `POST /membership/invitation` is an idempotent ensure operation: the invite page calls it on load so legacy members receive one missing permanent ordinary code, while registered members always receive the same value. It never revokes or regenerates an existing code.
...
- Versioned rule parameters use one typed contract shared by publication and runtime: `SELF_ORDER_TASK` accepts `minimumCompletedOrderAmountFen`, `["UPGRADE"]`, and an active `targetLevel`; `DIRECT_REFERRAL_TASK` accepts `requiredCompletedDirectReferrals`, `minimumReferralOrderAmountFen`, `["UPGRADE"]`, `requiredReferralLevel`, and active `targetLevel`; `DIRECT_REFERRAL_POINTS` accepts `qualificationCount`, `pointsStartOrdinal`, `totalPoints`, `availableAPoints`, `frozenBPoints`, `maxRewardDepth=1`, and `["UPGRADE"]`; `FROZEN_POINTS_RELEASE` accepts `["REPURCHASE"]`, `minimumCompletedOrderAmountFen`, `releaseMode=FIXED`, `releasePointsPerOrder`, and `batchOrder=FIFO`; `INACTIVITY_DOWNGRADE` accepts `inactiveMonths`, active `sourceLevel`, and active `targetLevel`; `ORDER_TIMERS` accepts canonical `autoReceiveDays`; the legacy `autoReceiveDaysAfterShipment` spelling is accepted only on persisted reads and is normalized away, while publication rejects it. It also accepts `afterSaleDaysAfterCompletion`, the three pending-order timeout fields, the four aftersale-stage timeout fields, `proofRetentionDays`, `maxProofFiles`, and `maxProofSizeBytes`. Timing fields are bounded to 1–365; proof retention remains 1–3650, and proof limits remain current capability configuration. Code/type pairs are exact and unknown fields are rejected for new publication.
...
- Existing ACTIVE rows retain the S3 in-memory repairs for documented V2 omissions (legacy sales-scene defaults and legacy point totals/depth). V19 forward-normalizes the four legacy aftersale-stage timer omissions before backfilling order snapshots; only proof-retention remains a persisted-read 180-day compatibility fallback. The three-field `DIRECT_REFERRAL_POINTS` shape is persisted-read compatibility only: publication rejects it, and persisted reads expose the canonical encoded fields after derivation. A malformed or unrepairable active version or order snapshot fails closed with a stable settings/runtime error and is never replaced by a fabricated default.
...
| Invitation issuer is not an ACTIVE account, has an inactive level, or has invitation disabled | Issue returns `INVITATION_NOT_ALLOWED`; consume returns `INVITE_CODE_INVALID`; no relation, profile, membership, ledger, or invitation-use write occurs |
| Old client calls invitation revoke or regenerate | HTTP 409 `INVITATION_IMMUTABLE`; do not lock or update invitation rows |
| An unresolved bootstrap invitation repair exists during new-member registration | HTTP 409 `BOOTSTRAP_INVITATION_REPAIR_REQUIRED`; do not inspect or consume any invitation. Existing-identity idempotency and sponsor claim remain available |
| Issued invitation is consumed after the issuer is disabled, locked, demoted, or otherwise loses invitation eligibility | Reject with `INVITE_CODE_INVALID`; revalidate the locked inviter context and preserve the existing relation graph |
...
| Persisted superior account or current membership level is not ACTIVE | HTTP 409 `SUPERIOR_UNAVAILABLE`; reject before order insertion and inventory reservation, then let the client refresh |
...
- Good: a new member enters only an invitation, clicks one registration button, receives a unique generated platform nickname plus one permanent ordinary invitation in the same local transaction, and reaches home with a nickname-initial avatar fallback.
...
- Base: a legacy member has no invitation; opening the invite page calls the idempotent POST ensure operation once, then displays the fixed code. A null ensure result displays a retry state and does not request wxacode.
...
- Bad: mutate an invitation from GET, revoke/regenerate a fixed ordinary code, return every historical rule version, trust a `userId` query parameter, or return a permanent RustFS URL.
...
- Invitation tests prove GET lookup is read-only, POST ensure fills only a missing permanent ordinary code, revoke/regenerate return `INVITATION_IMMUTABLE`, registration creates the member-owned code before consuming the input code, retries create no second code, and disabled/inactive owners fail locked consume validation without mutating their code.
...
- Registration adapter/mapper tests cover the full-publicId generated platform nickname, null avatar, permanent ordinary invitation creation order, insertion-failure rollback boundary, existing-identity idempotency, sponsor claim, local transaction annotation and stable duplicate-identity conflict before relation/invitation side effects. Once the external-identity lookup returns an existing account, it must be the adapter's only mapper interaction: no bootstrap-repair read, invitation lookup/lock, registration write, fixed-code insertion, usage increment or Bootstrap consume is allowed.
- Source-shape `ContractTest` classes may require script, migration and mapper-annotation fragments, but they are static contracts rather than executable Shell, Flyway/MySQL or MyBatis proof. Runtime migration, transaction, lock and concurrency conclusions require the Docker-backed Testcontainers suites; business-lifecycle conclusions require the manually enabled `workflow_dispatch.run_runtime_e2e` path. The default GitHub quality job intentionally does not install Java or run Maven, as specified in `container-delivery.md`.
...
- Order tests cover all three accepted source values, reject unknown sources before checkout, round-trip `buyerNote` through controller/application/domain/MyBatis/detail/auto-receive, apply V14 on an upgraded schema, allow an ACTIVE superior with a non-inviting level, reject a negative display price before checkout, reject a command/display mismatch with `PRICE_CHANGED` before `saveSubmitted`, reject an unavailable persisted superior with `SUPERIOR_UNAVAILABLE` before order/inventory writes, and reject a lock-vs-checkout-context mismatch with `PRICE_CHANGED` before any inventory row is reserved.
...
- Timeout tests cover `Order.timeoutClose` only from `PENDING_SHIPMENT`, per-order `ORDER_TIMERS` snapshot selection for pending and auto-receive deadlines, all four aftersale stage deadlines, missing/malformed snapshot fail-closed behavior, and `OrderTimeoutProcessor` routing all three due statuses through `persistTransition` so reserved inventory is released.
