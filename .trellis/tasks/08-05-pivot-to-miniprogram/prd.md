...
- 普通登录严格只提交 `wx.login` code；一次性 `sponsorClaimSecret` 只在显式发起人认领入口使用，不出现在普通邀请注册 UI。
...
- 公开注册页只显示邀请码输入和“一键注册”按钮；邀请码通过本地非空校验后取得一个全新 `wx.login` code，并向独立注册接口提交严格 JSON `{code, inviteCode}`。邀请码的有效性由后端在注册请求中判定。
- 注册不采集手机号、头像或昵称，不调用 `getPhoneNumber`、`chooseAvatar`、`getUserProfile` 或 `getUserInfo`，也不接受这些资料字段。`jscode2session` 只用于取得微信身份。
- 后端在单个本地事务内基于数据库唯一 `publicId` 生成唯一且明确的平台昵称 `宏杉会员-{publicId}`，保持 `avatarUrl=null`，并原子写入用户、外部身份、UnionID、上级关系、会员/积分账户、邀请次数、登录时间和发起人审计；Sa-Token 只在事务提交后创建。
- 网络或业务失败时客户端保留邀请码，但不得存储或重放登录 code；再次点击必须重新调用 `wx.login`。已绑定身份幂等返回既有资料，不改变上级、昵称或头像，也不重复计算邀请码。
- 后台会员列表/详情显示权威平台昵称；空头像或加载失败时使用昵称首字，不用品牌 Logo 冒充会员头像。
- `pages/profile/edit` 仅作为“我的”中的可选资料编辑入口；注册成功和已有会员登录均直接进入首页，不强制跳转资料页。
- 发起人认领仅通过 `pages/register/register?mode=sponsor` 显式打开；认领密钥由用户在该模式内输入，不进入 URL，不在普通邀请注册页提供切换入口。
- 用户后续主动编辑头像/昵称时，微信公众平台隐私保护指引必须声明其会员资料展示用途；注册本身不请求这些能力。
...
- 覆盖全部 26 个已注册页面，优先重构首页、分类、搜索、商品详情、购物车、结算、订单、售后、会员、个人中心与可选资料编辑；内容/规则等只读页也必须使用同一设计系统。
...
12. 26 个小程序页面具有一致品牌主题与清晰商城层级，核心交易页具备加载、空、错误/重试、禁用和安全区状态；登录、可选资料编辑、加购、结算、确认、发货、收货、售后与客服契约不回归。
13. 小程序静态契约测试能识别 FirstUI 组件与工具文件，检查页面/公共组件注册、WXML 事件和必需资产；上传包保持在微信主包限制内，`pnpm test:miniprogram` 通过。
14. 新注册会员只填写邀请码并点击“一键注册”；客户端以全新 `wx.login` code 提交 `{code, inviteCode}`，后端原子创建唯一 `宏杉会员-{publicId}` 平台昵称、空头像和完整会员关系。失败不留半成品账号，使用新 code 重试不重复计数。
15. V15 前向迁移兼容旧会员空资料，并允许真实 `WECHAT_MP` 发起人认领通过数据库约束。
16. 已有会员登录成功后直接进入首页；头像/昵称编辑只由“我的”中的明确入口触发，保留同昵称 no-op、CAS 冲突刷新和昵称成功后头像单独重试能力。
