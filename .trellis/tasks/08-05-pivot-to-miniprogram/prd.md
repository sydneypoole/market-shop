...
- 登录页继续只交换 `wx.login` 临时 code；注册页使用微信当前的 `chooseAvatar`、`input type="nickname"` 与 `getPhoneNumber` 能力采集会员头像、注册昵称和已验证手机号。
- `getPhoneNumber` 动态 code 只能由后端调用微信接口消费；客户端、URL、storage、日志和管理后台分页均不得携带原始手机号。数据库保存后端生成的脱敏手机号与验证时间。
- 注册成功取得会员 Token 后，在注册页继续提交权威资料和上传临时头像。资料或头像失败只重试未完成阶段，不能重放已消费的手机号 code、邀请码或一次性发起人认领密钥。
- 头像通过 identity 语义的存储 port 永久化，local 与 S3/RustFS provider 行为一致；仅保存自有对象引用并通过稳定同源端点读取，不保存 `wxfile://` 临时路径或任意外部头像 URL。
- 后台会员列表和详情显示头像、注册昵称和脱敏手机号；头像加载失败退化为昵称首字，不使用品牌 Logo 冒充会员头像。
- 小程序个人中心使用权威会员头像和昵称；旧会员资料为空时保持兼容降级。
- 新的前向 Flyway 迁移补齐资料元数据，并修复发起人认领约束以允许当前真实小程序 provider `WECHAT_MP`，不得修改已执行的 V1/V11 文件。
- 微信公众平台发布前必须在用户隐私保护指引中声明头像、昵称和手机号用途；未完成声明或用户未同意时，页面明确提示而不是静默失败。

### P1：每次登录后确认头像与昵称

- 已有会员每次通过登录页成功交换新的 `wx.login` code 并写入 Token 后，进入独立的“确认头像与昵称”页；登录请求仍只提交 code，不携带注册凭据、phone code 或临时头像。
- 确认页先读取 `/membership/me` 的权威资料，仅通过微信当前的 `chooseAvatar` 和 `input type="nickname"` 让用户主动更新；不调用已回收的用户资料 API，不声称能静默同步微信头像链接。
- 该页必须提供“暂不更新，进入商城”和返回首页能力；跳过、无变化提交和保存成功都使用 `switchTab` 进入首页，不在登录/资料页之间堆栈。
- 昵称通过新的会员会话保护接口独立更新，严格保留已验证的脱敏手机号和验证时间；同昵称是 no-op，不增加 version。头像继续复用已有的 multipart 安全上传接口。
- 昵称与头像分阶段保存：昵称成功后头像失败时，重试只上传头像；所有操作有 loading/disabled 防重，临时头像路径不进入 URL、storage、JSON 或日志。
- 个人中心提供同一资料确认页的入口；用户从个人中心主动进入时不得索取手机号或触发登录。
...
- 覆盖全部 26 个已注册页面，优先重构首页、分类、搜索、商品详情、购物车、结算、订单、售后、会员、个人中心与资料确认；内容/规则等只读页也必须使用同一设计系统。
...
12. 26 个小程序页面具有一致品牌主题与清晰商城层级，核心交易页具备加载、空、错误/重试、禁用和安全区状态；登录、资料确认、加购、结算、确认、发货、收货、售后与客服契约不回归。
13. 小程序静态契约测试能识别 FirstUI 组件与工具文件，检查页面/公共组件注册、WXML 事件和必需资产；上传包保持在微信主包限制内，`pnpm test:miniprogram` 通过。
14. 新注册会员在资料完善后，后台会员列表/详情可看到自有稳定头像、注册昵称和后端脱敏手机号；小程序不提交原始手机号，头像临时路径不落库，profile/avatar 任一步失败均可单独重试。
15. V15 前向迁移兼容旧会员空资料，并允许真实 `WECHAT_MP` 发起人认领通过数据库约束。
16. 已有会员每次完成登录都进入可跳过的头像/昵称确认页；页面不索取手机号，同昵称不写库，昵称成功后头像重试不重复更新昵称，跳过/成功均可回首页。
