...
- 当前 26 个页面、4 个公共组件，所有业务请求、路由、权限、幂等、凭证和冲突恢复都在页面 JS/API utils 中。迁移应以 WXML/WXSS 与业务 wrapper 为主，不使用 FirstUI 接管业务请求、上传或路由。
...
- 26 个注册页面的 JSON/WXML/WXSS 全部迁移并真实使用 `brand-shell`；原生 tabBar/导航/客服/地区选择/上传/预览保留。
...
- 按 `project.config.json` `packOptions.ignore` 排除 `tests/` 和 `README.md` 后，主包源文件为 203 个、1116913 bytes（1.065 MiB）；License 与版本说明继续随源码分发。
- `pnpm test:miniprogram` 现包含 58 项 API/运行时/幂等/FirstUI 事件/地址加载/库存闭合/角色门禁/签名凭证竞态/资料冲突恢复/状态色/静态/包体契约。本地微信 `wcc`/`wcsc` 分别通过 41 个 WXML 与 44 个 WXSS；微信开发者工具已实际渲染首页、商品详情与个人中心，编辑器 Problems 为 0。
