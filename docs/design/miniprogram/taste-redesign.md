...
- 微信登录、注册和已有会员资料确认保持独立页面，`market-shop-user-token`、401 清理并回到登录页；已有会员完成新的登录后先进入可跳过的资料确认页，完成或跳过后返回公共首页；
...
静态门禁检查页面/组件注册、WXML 事件、FirstUI 来源与许可证、主题映射、26 页图标体系、tab PNG 规格和业务消费者契约。上传源文件按 `project.config.json` 的 `packOptions.ignore` 计算，Taste 设计改动目标小于 1.4 MiB，主包必须小于 2 MiB。
